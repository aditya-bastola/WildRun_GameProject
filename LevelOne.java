...
public class LevelOne extends JPanel implements KeyListener, ActionListener
{
	...
	JButton TO_LEVEL_TWO = new JButton ("");
	...
	ImageIcon brickIcon2 = new ImageIcon("jungle_block.png");
	Image brick12 = brickIcon2.getImage();
...
ImageIcon level2ButtonIcon = new ImageIcon("level_2_button.png");
	...
	ImageIcon underWater = new ImageIcon("underWaterBackground.png");
	...
	public LevelOne(CardLayout cardLayout, cardLayoutPanel cards)
	{
		super();
		...
		this.addComponentListener( new ComponentAdapter() {
			...
			public void componentShown( ComponentEvent e ) {
				LevelOne.this.requestFocusInWindow();

			}
		});

		TO_LEVEL_TWO.setSize(200,75);

		TO_LEVEL_TWO.setOpaque(false);
		TO_LEVEL_TWO.setContentAreaFilled(false);
		TO_LEVEL_TWO.setBorderPainted(false);

		this.add(TO_LEVEL_TWO);

		TO_LEVEL_TWO.addActionListener(e -> cardLayout.next(cards));

		cards.add(this);

		for (int i = 1; i < 11; i++)
		{
			ImageIcon coin = new ImageIcon("goldbar" + i + ".png");
		...
		csvArray = CSVReader.readCSV("LevelOneMap.csv");
			...
			for (int n = 0; n < csvArray[0].length; n++)
			{
				if (csvArray [m] [n] != 0)
				{
					...
					if (csvArray [m] [n] == 51) { // entry portal - vertical - left
						...
						// portal 1
						if (n == 14) {
						sprite1.setPortalX(27 * 50);
						sprite1.setPortalY(3 * 50);
						sprite1.setPlayerPortalX_velocity(0);
						sprite1.setPlayerPortalY_velocity(0);
						
						System.out.println("Start, X" + sprite1.getPortalX() + "Y" + sprite1.getPortalY());
						}
					...
					if (csvArray [m] [n] == 53) { // entry portal - horizontal up
						...
						System.out.println("53portal n: " + n);
						
						// portal 2
						if (n == 49) {
							sprite1.setEntryPortal(true);
							System.out.println("2ndportal");
							sprite1.setPortalX(62 * 50);
							sprite1.setPortalY(4 * 50);
							sprite1.setPlayerPortalX_velocity(0);
						...
						// portal 3 
						if (n == 76) {
							System.out.println("portal at 76");
							sprite1.setEntryPortal(true);
							System.out.println("2ndportal");
							sprite1.setPortalX(88 * 50);
							sprite1.setPortalY(8 * 50);
							sprite1.setPlayerPortalX_velocity(7);
							sprite1.setPlayerPortalY_velocity(7);
	...
	public void paintComponent (Graphics g) {
		...
		for (int i = 0 - viewX - 70 - 1200; i < 1200* 20 - viewX; i += 1200)
		{
			g.drawImage(background1, i, 0, 1200, 675, null);
		}
...
TO_LEVEL_TWO.setLocation(viewY + 900, viewY + 15);

		RESET.setLocation(viewY + 600, viewY + 15);

		textMaker("Coins: " + coinsNum, 350, 35, g);

		if (p1.isJetpack() == true)
			textMaker("Fuel: " + jetPackShots, 500, 35, g);

		if (gameOver) {
			g.drawImage(level2Button, viewY + 900, viewY + 15, 200,75,null);
			g.drawImage(RESET_IMAGE1, viewY + 600, viewY + 15, 200,75,null);
		...
		if (p1.getDIRECTION() == 2 && p1.isJetpack()) {
			g.drawImage(jetPackRight1,p1.getx_center() - 27, p1.gety_center() - 15,25,25, null);
		}
		else if (p1.getDIRECTION() == 3 && p1.isJetpack()) {
			g.drawImage(jetPackLeft1,p1.getx_center(), p1.gety_center() - 15,25,25, null);
		}
	...
	public void deathCheck() {
		...
		if (fallDown)
		{
			...
			deadX = p1.getLeft();
			deadY = p1.getTop();
			System.out.println("deadX" + deadX + "deadY" + deadY);
			bloodCounter = 0;
			bloodAnimation = true;
			bloodX = p1.getLeft();
			bloodY = p1.getTop();
